...
static const struct locale_support_t {
	...
	{ "C",           __ELIBC_ASCII, { "", ".", "", "", "", "", "", "", "", "", CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX} },
	{ "POSIX",       __ELIBC_ASCII, { "", ".", "", "", "", "", "", "", "", "", CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX} },
	{ "en_US",       __ELIBC_ASCII, { "$", ".", "", "USD", ".", "", ",", "-", "", ",", 2, 2, 1, 0, 1, 1, 1, 1} },
	{ "en_US.UTF-8", __ELIBC_UTF8,  { "$", ".", "", "USD", ".", "", ",", "-", "", ",", 2, 2, 1, 0, 1, 1, 1, 1} },
	{ 0,             __ELIBC_ASCII, { "", "",  "", "", "", "", "", "", "", "", CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX, CHAR_MAX} }
};
...
char *setlocale(int category, const char *locname) {
	...
	if(locname) {
		...
		if(locname[0] == '\0') {
			switch(category) {
			case LC_COLLATE:
				return __elibc_setlocale(current_lconv, LC_COLLATE,  getenv("LC_COLLATE"),  locale_name, sizeof(locale_name));
			case LC_CTYPE:
				return __elibc_setlocale(current_lconv, LC_CTYPE,    getenv("LC_CTYPE"),    locale_name, sizeof(locale_name));
			case LC_MONETARY:
				return __elibc_setlocale(current_lconv, LC_MONETARY, getenv("LC_MONETARY"), locale_name, sizeof(locale_name));
			case LC_NUMERIC:
				return __elibc_setlocale(current_lconv, LC_NUMERIC,  getenv("LC_NUMERIC"),  locale_name, sizeof(locale_name));
			case LC_TIME:
				return __elibc_setlocale(current_lconv, LC_TIME,     getenv("LC_TIME"),     locale_name, sizeof(locale_name));
			case LC_ALL:
				{
					...
					if(!ret) {
						...
						if(!ret) {
							...
							if(!ret) {
								ret = __elibc_setlocale(current_lconv, LC_ALL, getenv("_LOCALE"), locale_name, sizeof(locale_name));
							}
						}
					}
			...
			return __elibc_setlocale(current_lconv, category, locname, locale_name, sizeof(locale_name));
		}
	} else {
		if(category == LC_ALL) {

			if(__elibc_categories_equal()) {
				snprintf(locale_name, sizeof(locale_name), "%s", __elibc_locale_meta[__ELIBC_LC_ALL].name);
			} else {
				snprintf(locale_name, sizeof(locale_name), "%s;LC_COLLATE=%s;LC_CTYPE=%s;LC_MONETARY=%s;LC_NUMERIC=%s;LC_TIME=%s",
			...
			switch(category) {
			case LC_COLLATE:
				strlcpy(locale_name, __elibc_locale_meta[__ELIBC_LC_COLLATE].name, sizeof(locale_name));
				break;
			case LC_CTYPE:
				strlcpy(locale_name, __elibc_locale_meta[__ELIBC_LC_CTYPE].name, sizeof(locale_name));
				break;
			case LC_MONETARY:
				strlcpy(locale_name, __elibc_locale_meta[__ELIBC_LC_MONETARY].name, sizeof(locale_name));
				break;
			case LC_NUMERIC:
				strlcpy(locale_name, __elibc_locale_meta[__ELIBC_LC_NUMERIC].name, sizeof(locale_name));
				break;
			case LC_TIME:
				strlcpy(locale_name, __elibc_locale_meta[__ELIBC_LC_TIME].name, sizeof(locale_name));
				break;
			default:
				return 0;
			}
