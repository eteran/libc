...
/* we use this because the our assert uses fprintf to print its message, resulting
 * in an infinite recursion if some assertions fail
 */
#if defined NDEBUG
...
// Name:
//----------------------------------------------------------------------------*/
char *_format_float_decimal(char *buf, size_t sz, double value, int precision, char format, uint8_t flags) {
...
// Name:
//----------------------------------------------------------------------------*/
char *_format_float_exponent(char *buf, size_t sz, double value, int precision, char format, uint8_t flags) {
		...
		if(*format == '%') {
			...
			do_float:
				...
				if(modifier == MOD_LONG_DOUBLE) {
					...
					s_ptr = _format_float(flt_buf, sizeof(flt_buf), va_arg(aq, double), precision, ch, flags);
				}
#else
				s_ptr = "0.0";
#endif
			...
			case 'b': /* extension, BINARY mode */
				...
				case MOD_CHAR:
					...
					break;
				case MOD_SHORT:
					s_ptr = _unsigned_itoa(num_buf, sizeof(num_buf), ch, precision, (unsigned short int)va_arg(aq, unsigned int), width, flags);
					break;
				case MOD_LONG:
					s_ptr = _unsigned_itoa(num_buf, sizeof(num_buf), ch, precision, va_arg(aq, unsigned long int), width, flags);
					break;
				case MOD_LONG_LONG:
					s_ptr = _unsigned_itoa(num_buf, sizeof(num_buf), ch, precision, va_arg(aq, unsigned long long int), width, flags);
					break;
				case MOD_INTMAX_T:
					s_ptr = _unsigned_itoa(num_buf, sizeof(num_buf), ch, precision, va_arg(aq, uintmax_t), width, flags);
					break;
				case MOD_SIZE_T:
					s_ptr = _unsigned_itoa(num_buf, sizeof(num_buf), ch, precision, va_arg(aq, size_t), width, flags);
					break;
				case MOD_PTRDIFF_T:
					/* NOTE: we really want uptrdiff_t or something to that effect, but unsigned long will have to do */
					s_ptr = _unsigned_itoa(num_buf, sizeof(num_buf), ch, precision, (unsigned long)va_arg(aq, ptrdiff_t), width, flags);
					break;
				default:
			...
			case 'd':
				...
				case MOD_CHAR:
					...
					break;
				case MOD_SHORT:
					s_ptr = _signed_itoa(num_buf, sizeof(num_buf), ch, precision, (short int)va_arg(aq, int), width, flags);
					break;
				case MOD_LONG:
					s_ptr = _signed_itoa(num_buf, sizeof(num_buf), ch, precision, va_arg(aq, long int), width, flags);
					break;
				case MOD_LONG_LONG:
					s_ptr = _signed_itoa(num_buf, sizeof(num_buf), ch, precision, va_arg(aq, long long int), width, flags);
					break;
				case MOD_INTMAX_T:
					s_ptr = _signed_itoa(num_buf, sizeof(num_buf), ch, precision, va_arg(aq, intmax_t), width, flags);
					break;
				case MOD_SIZE_T:
					s_ptr = _signed_itoa(num_buf, sizeof(num_buf), ch, precision, va_arg(aq, ssize_t), width, flags);
					break;
				case MOD_PTRDIFF_T:
					s_ptr = _signed_itoa(num_buf, sizeof(num_buf), ch, precision, va_arg(aq, ptrdiff_t), width, flags);
					break;
				default:
